...
const container = dv.el("div", "", { 
    style: "background: var(--background-primary); border: 1px solid var(--background-modifier-border); border-radius: 8px; padding: 20px; font-family: sans-serif; color: var(--text-normal);" 
});

container.innerHTML = `
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
        <h3 style="margin: 0;">Morris Traversal 視覺化 (純 JS 版)</h3>
        <div>
            <button id="morris-play" style="padding: 4px 12px; cursor: pointer;">播放</button>
            <button id="morris-next" style="padding: 4px 12px; cursor: pointer;">下一步</button>
            <button id="morris-reset" style="padding: 4px 12px; cursor: pointer;">重置</button>
        </div>
    </div>
    <div id="canvas-container" style="background: var(--background-secondary); border-radius: 6px; display: flex; justify-content: center; padding: 20px; position: relative;">
        <svg id="morris-svg" width="400" height="280" style="overflow: visible;"></svg>
    </div>
    <div id="morris-log" style="background: #1e1e1e; color: #a6e22e; padding: 10px; border-radius: 4px; font-family: monospace; font-size: 13px; margin: 15px 0; min-height: 40px;">
        點擊「下一步」開始演算法...
    </div>
    <div style="font-size: 14px; font-weight: bold;">
        結果: <span id="morris-result" style="color: var(--text-accent);">[]</span>
    </div>
`;
...
// 預先計算所有步驟 (Morris Traversal 邏輯)
function generateSteps() {
    let s = [];
    let res = [];
    let curr = JSON.parse(JSON.stringify(tree));
    
    let c = curr;
    while (c) {
        s.push({ type: 'check', node: c.val, msg: `檢查節點 ${c.val}`, res: [...res] });
        if (!c.left) {
            res.push(c.val);
            s.push({ type: 'visit', node: c.val, msg: `${c.val} 無左子樹，記錄結果並移至右側`, res: [...res] });
            c = c.right;
        } else {
            let pre = c.left;
            ...
            if (!pre.right) {
                pre.right = c;
                s.push({ type: 'thread', node: c.val, pre: pre.val, msg: `建立線索：${pre.val} -> ${c.val}`, res: [...res], thread: {from: pre.val, to: c.val, action: 'add'} });
                c = c.left;
            } else {
                pre.right = null;
                res.push(c.val);
                s.push({ type: 'unthread', node: c.val, pre: pre.val, msg: `斷開線索，記錄 ${c.val} 並移至右側`, res: [...res], thread: {from: pre.val, to: c.val, action: 'remove'} });
                c = c.right;
            }
        }
    }
    s.push({ type: 'done', msg: "遍歷完成！", res: [...res] });
    return s;
}
...
// SVG 繪製函數
function drawTree() {
    const svg = container.querySelector("#morris-svg");
    svg.innerHTML = "";
    const nodeRadius = 18;
    const positions = {};

    function getPos(node, x, y, dx) {
        if (!node) return;
        positions[node.val] = { x, y };
        if (node.left) {
            const lx = x - dx, ly = y + 60;
            svg.innerHTML += `<line x1="${x}" y1="${y}" x2="${lx}" y2="${ly}" stroke="gray" stroke-width="1.5" />`;
            getPos(node.left, lx, ly, dx / 1.7);
        }
        if (node.right) {
            const rx = x + dx, ry = y + 60;
            svg.innerHTML += `<line x1="${x}" y1="${y}" x2="${rx}" y2="${ry}" stroke="gray" stroke-width="1.5" />`;
            getPos(node.right, rx, ry, dx / 1.7);
        }
    }
    getPos(tree, 200, 30, 100);

    Object.entries(positions).forEach(([val, pos]) => {
        svg.innerHTML += `
            <circle id="node-${val}" cx="${pos.x}" cy="${pos.y}" r="${nodeRadius}" fill="white" stroke="#666" stroke-width="2" />
            <text x="${pos.x}" y="${pos.y}" dy="5" text-anchor="middle" style="font-size:12px; font-weight:bold; pointer-events:none;">${val}</text>
        `;
    });
    return positions;
}

const nodePositions = drawTree();
const svg = container.querySelector("#morris-svg");
...
function updateUI() {
    ...
    // 更新日誌與結果
    container.querySelector("#morris-log").textContent = step.msg;
    container.querySelector("#morris-result").textContent = `[${step.res.join(", ")}]`;
    ...
    // 高亮當前節點
    if (step.node) {
        const circle = svg.querySelector(`#node-${step.node}`);
        if (circle) circle.setAttribute("fill", "#3b82f6");
    }

    // 更新線索 (Threads)
    // 這裡為了簡單，直接根據步驟顯示線索
    svg.querySelectorAll(".thread-line").forEach(l => l.remove());
    let activeThreads = [];
    for(let i=0; i<=currentStep; i++) {
        if (steps[i].thread) {
            if (steps[i].thread.action === 'add') activeThreads.push(steps[i].thread);
            else activeThreads = activeThreads.filter(t => t.from !== steps[i].thread.from);
        }
    }
...
// 按鈕事件
container.querySelector("#morris-next").addEventListener("click", () => {
...
container.querySelector("#morris-reset").addEventListener("click", () => {
    currentStep = -1;
    isPlaying = false;
    clearInterval(timer);
    container.querySelector("#morris-log").textContent = "已重置。";
    container.querySelector("#morris-result").textContent = "[]";
    svg.querySelectorAll("circle").forEach(c => c.setAttribute("fill", "white"));
    svg.querySelectorAll(".thread-line").forEach(l => l.remove());
});

container.querySelector("#morris-play").addEventListener("click", () => {
    if (isPlaying) {
        clearInterval(timer);
        isPlaying = false;
        container.querySelector("#morris-play").textContent = "播放";
    } else {
        isPlaying = true;
        container.querySelector("#morris-play").textContent = "暫停";
        timer = setInterval(() => {
            if (currentStep < steps.length - 1) {
                currentStep++;
                updateUI();
