...
## Morris Traversal 演算法動畫模擬 <iframe src="assets/MorrisTraversal.html" width="100%" height="500px" frameborder="0"> </iframe> ### 為什麼這樣做？ * **兼容性**：Obsidian 的編輯器本身不直接運行 React/JSX。透過 `iframe` 嵌入一個獨立的 HTML 檔案，可以確保所有的腳本（React, Tailwind）都能正常運作，且不會影響到 Obsidian 自身的 UI。 * **效能**：動畫邏輯被封裝在單一檔案中，讀取速度快且易於管理。 * **美觀**：這個檔案自帶了 Tailwind CSS 渲染，呈現效果與開發環境一致。 如果你想要直接在 `DataviewJS` 中寫死邏輯（不使用外部檔案），那會需要用到 `dv.el` 來手動建立複雜的 DOM 結構，對於這種高度互動的動畫，使用 HTML + Iframe 是最推薦的做法。需要調整動畫速度或樹的結構嗎？
---
