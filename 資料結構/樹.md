...
const IMG_GEN_REPLACE_MAP = {"official portrait of Claude Shannon information theory&reimagine=1": "https://lh3.googleusercontent.com/gg/AIJ2gl89i5l3JbicaWOFDTrqVLYAg295b_cdfgoX_JJoZWvgtgXQQbv-Q9Uzi7TBg6QjSm8N9sNMtEJJ0Hf6CCg29TeoxpV4EZ8CUE7c9SQ__-i-H51I_xiCsELaxAb91OB7YU3P2OhA1Xl5jJg2sF5i_I8yoH7y70DwjKFD9hl3kumEtqDOb93DverYlYKiIIBFelWawKS1cYvdGcRV4qZ3PzJ1EGOlL-WyGNUmft9PeOgGpSytIQe0SMzae4TNiKaDc6O4hnk-E0xHPq4h898oUNHjfUotJEUJOcTJEj90ht_T0C2YnI8FAjTazGYTI9r5zSzR3G1tG5O0Bh9abecjiwr0", "this image unchanged&context_image_idx=0": "資料結構/assets/huffman/huffman_01_fixed_length.png", "abstract binary tree network pattern blue lines": "資料結構/assets/huffman/huffman_03_tree_growth.png", "a clean minimalist visualization of data compression from many boxes into a small efficient box&aspect=4:3": "資料結構/assets/huffman/huffman_02_compression.png", "abstract mathematical concept growth of tree nodes connecting glowing points&aspect=16:9": "資料結構/assets/huffman/huffman_03_tree_growth.png", "minimalist diagram showing entropy and efficiency curve data compression&aspect=16:9": "資料結構/assets/huffman/huffman_04_entropy.png"};
...
const localPaths = ["資料結構/assets/huffman/AF2bZyjKMgp8hYtrz8uwlAs5nm2p_PlQ7vIqFy1VAru7NqEk7G.jpg", "資料結構/assets/huffman/huffman_01_fixed_length.png", "資料結構/assets/huffman/huffman_02_compression.png", "資料結構/assets/huffman/huffman_03_tree_growth.png", "資料結構/assets/huffman/huffman_04_entropy.png", "資料結構/assets/huffman/img_57f320b3bb.jpg", "資料結構/assets/huffman/img_b619eb1bb8.jpg", "資料結構/assets/huffman/unnamed(1).jpg", "資料結構/assets/huffman/unnamed(2).jpg", "資料結構/assets/huffman/unnamed(3).jpg", "資料結構/assets/huffman/unnamed(4).jpg", "資料結構/assets/huffman/unnamed(5).jpg", "資料結構/assets/huffman/unnamed.jpg"];
...
iframe.srcdoc = finalHtml;
iframe.className = "seamless-iframe"; // 使用 CSS 類別控制樣式
iframe.scrolling = "no";

dv.container.appendChild(iframe);

// 使用防抖動 (debounce) 來避免過度頻繁調整
let resizeTimeout;
const safeResize = () => {
    if (resizeTimeout) clearTimeout(resizeTimeout);
    resizeTimeout = setTimeout(() => {
        if (iframe.contentWindow && iframe.contentWindow.document && iframe.contentWindow.document.body) {
            const bodyH = iframe.contentWindow.document.body.scrollHeight;
            const docH = iframe.contentWindow.document.documentElement.scrollHeight;
            let h = Math.max(bodyH, docH);
            
            const currentH = parseInt(iframe.style.height || "0");
            if (Math.abs(h - currentH) > 5) {
...
iframe.onload = () => {
    ...
    if (iframe.contentWindow.ResizeObserver) {
        const ro = new iframe.contentWindow.ResizeObserver(safeResize);
        if (iframe.contentWindow.document.body) {
           ro.observe(iframe.contentWindow.document.body);
        }
    } else {
         const observer = new MutationObserver(safeResize);
         if (iframe.contentWindow.document.body) {
    ...
    setTimeout(safeResize, 2000);
};
```
### 📚 哈夫曼樹核心觀念
**1. 定義**
- **路徑長度**：從根節點到該節點的路徑分支數目。
- **帶權路徑長度 (WPL)**：所有葉子節點的「權值 $\times$ 路徑長度」之總和。即 $WPL = \sum_{i=1}^n w_i l_i$。
- **哈夫曼樹/最佳二元樹**：在所有含 $n$ 個帶權葉子節點的二元樹中，**WPL 最小**的那一棵。

**2. 建構演算法 (Greedy Strategy)**
1.  **選二小**：在森林中選出根節點權值最小的兩棵樹。
2.  **合成新**：新增一個根節點，其權值為兩棵子樹權值之和。
3.  **刪舊加新**：從森林中移除選中的兩棵樹，將新生成的樹加入森林。
4.  **重複**：重複上述步驟，直到森林中只剩下一棵樹。

**3. 重要性質**
- 只有度為 0 和度為 2 的節點，**沒有度為 1 的節點**。
- 若有 $n$ 個葉子節點，則總節點數為 $2n - 1$。
- 哈夫曼編碼（前綴碼）：任一字符的編碼都不是另一字符編碼的前綴，保證解碼無歧義。

#### 💻 C 語言實作範例 (Huffman Construction)
```c
typedef struct {
    int weight;
...
void CreateHuffmanTree(HuffmanTree *HT, int *weights, int n) {
    ...
    *HT = (HuffmanTree)malloc((m + 1) * sizeof(HTNode)); // 0 號單元不用
    ...
    for (int i = 1; i <= n; ++i) (*HT)[i].weight = weights[i - 1];

    // 建構 (n+1 ~ m)
    for (int i = n + 1; i <= m; ++i) {
        int s1, s2;
        Select(HT, i - 1, &s1, &s2); // 自定義函數：在前 i-1 個節點中選權值最小且 parent==0 的兩個
...
> **並查集**是一種用於管理元素分組（不相交集合）的高效資料結構。
> 核心應用：連通性檢測、Krusal 最小生成樹演算法、最近公共祖先 (LCA)。

## 📝 核心操作小結
| 操作 | 功能 | 時間複雜度 (優化後) |
| :--- | :--- | :--- |
| **MakeSet** | 初始化，每個元素自成一個集合 | $O(1)$ |
| **Find** | 查找元素所屬的集合代表 (根節點) | $O(\alpha(n)) \approx O(1)$ |
| **Union** | 合併兩個元素所屬的集合 | $O(\alpha(n)) \approx O(1)$ |

> **優化關鍵**：
> 1. **路徑壓縮 (Path Compression)**：在 Find 過程中將路徑上所有節點直接掛到根下，扁平化樹高。
> 2. **按秩合併 (Union by Rank)**：將矮樹合併到高樹下，防止樹退化成鏈。
...
const safeResize = () => {
    ...
    resizeTimeout = setTimeout(() => {
        if (iframe.contentWindow && iframe.contentWindow.document && iframe.contentWindow.document.body) {
            // 使用 body.scrollHeight 通常比較準確，避免 documentElement 包含 iframe 自身的高度增長
            const bodyH = iframe.contentWindow.document.body.scrollHeight;
            const docH = iframe.contentWindow.document.documentElement.scrollHeight;
            let h = Math.max(bodyH, docH);
            
            // 只有當高度變化超過 5px 時才調整，避免微小震盪
            const currentH = parseInt(iframe.style.height || "0");
            if (Math.abs(h - currentH) > 5) {
                iframe.style.height = (h + 30) + "px"; // 減少緩衝區
            }
        }
    }, 100);
};
...
iframe.onload = () => {
    ...
    // 使用 ResizeObserver 監聽 body 大小變化，比 MutationObserver 更安全
    if (iframe.contentWindow.ResizeObserver) {
        const ro = new iframe.contentWindow.ResizeObserver(safeResize);
        if (iframe.contentWindow.document.body) {
           ro.observe(iframe.contentWindow.document.body);
         ...
         // Fallback for older browsers (unlikely in Obsidian)
         const observer = new MutationObserver(safeResize);
         if (iframe.contentWindow.document.body) {
            observer.observe(iframe.contentWindow.document.body, { 
                childList: true, subtree: true, attributes: false // 減少監聽屬性變化
            });
         }
    }
