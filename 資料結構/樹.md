...
<body>
    ...
    <script type="text/babel">
        ...
        const initialTree = {
            ...
            right: { id: '3', val: 3, left: null, right: { id: '6', val: 6, left: null, right: null } }
        };
        ...
        const App = () => {
            ...
            useEffect(() => {
                const s = []; let c = initialTree; let curR = []; let curT = [];
                const getR = (n, t) => { 
                    const found = t.find(i => i.from === n.id);
                    return found ? findNode(initialTree, found.to) : n.right;
                };
                const findNode = (r, id) => {
                ...
                while (c) {
                    s.push({ c: c.id, p: null, t: [...curT], r: [...curR], m: "檢查節點 " + c.val });
                    if (!c.left) {
                        curR.push(c.val);
                        s.push({ c: c.id, p: null, t: [...curT], r: [...curR], m: c.val + " 無左子，加入結果" });
                        c = getR(c, curT);
                    } else {
                        let p = c.left;
                        while (getR(p, curT) && getR(p, curT).id !== c.id) p = getR(p, curT);
                        if (!getR(p, curT)) {
                            curT.push({ from: p.id, to: c.id });
                            s.push({ c: c.id, p: p.id, t: [...curT], r: [...curR], m: "建立線索: " + p.val + " -> " + c.val });
                            c = c.left;
                        } else {
                            curT = curT.filter(i => !(i.from === p.id && i.to === c.id));
                            curR.push(c.val);
                            s.push({ c: c.id, p: p.id, t: [...curT], r: [...curR], m: "斷開線索，加入 " + c.val });
                            c = getR(c, curT);
                        }
                    }
                }
                s.push({ c: null, p: null, t: [], r: [...curR], m: "完成！" });
                setSteps(s);
            }, []);
            ...
            const next = () => {
                if (idx < steps.length - 1) {
                    const s = steps[idx + 1];
                    setIdx(idx + 1); setCurrentId(s.c); setPreId(s.p); setThreads(s.t); setRes(s.r);
                } else setPlaying(false);
            };

            useEffect(() => {
                if (playing) { const t = setTimeout(next, 800); return () => clearTimeout(t); }
            }, [playing, idx]);

            const findP = (n, id, x, y, dx) => {
                if (!n) return null; if (n.id === id) return { x, y };
                return findP(n.left, id, x-dx, y+60, dx/1.8) || findP(n.right, id, x+dx, y+60, dx/1.8);
            };

            const drawNode = (n, x, y, dx) => {
                if (!n) return null;
                const isC = currentId === n.id; const isP = preId === n.id;
                return (
                    <g key={n.id}>
                        {n.left && <line x1={x} y1={y} x2={x-dx} y2={y+60} stroke="#cbd5e1" strokeWidth="2" />}
                        {n.right && <line x1={x} y1={y} x2={x+dx} y2={y+60} stroke="#cbd5e1" strokeWidth="2" />}
                        <circle cx={x} cy={y} r="18" fill={isC ? '#3b82f6' : (isP ? '#f59e0b' : '#fff')} stroke="#64748b" strokeWidth="2" className="node-circle" />
                        <text x={x} y={y} dy="5" textAnchor="middle" fill={isC || isP ? "white" : "#334155"} style={{fontSize: '12px', fontWeight: 'bold'}}>{n.val}</text>
                        {drawNode(n.left, x-dx, y+60, dx/1.8)}
                        {drawNode(n.right, x+dx, y+60, dx/1.8)}
                    </g>
                );
            };

            return (
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 shadow-sm h-full flex flex-col font-sans">
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="text-sm font-bold text-slate-700">Morris Traversal 視覺化</h2>
                        <div className="flex gap-2">
                            <button onClick={() => setPlaying(!playing)} className="px-3 py-1 bg-blue-600 text-white text-xs rounded shadow-sm">{playing ? '暫停' : '播放'}</button>
                            <button onClick={next} className="px-3 py-1 bg-white border border-slate-200 text-xs rounded">下一步</button>
                        </div>
                    </div>
                    <div className="flex-1 bg-white rounded-lg border border-slate-100 flex justify-center items-center overflow-hidden mb-4">
                        <svg width="100%" height="240" viewBox="0 0 400 300">
                            {drawNode(initialTree, 200, 40, 100)}
                            {threads.map((t, i) => {
                                const f = findP(initialTree, t.from, 200, 40, 100);
                                const to = findP(initialTree, t.to, 200, 40, 100);
                                return f && to ? <path key={i} d={"M"+f.x+" "+f.y+" Q"+(Math.max(f.x, to.x)+40)+" "+((f.y+to.y)/2)+" "+to.x+" "+to.y} fill="none" stroke="#10b981" strokeWidth="2" strokeDasharray="4" /> : null;
                            })}
                        </svg>
                    </div>
                    <div className="text-xs bg-slate-900 text-blue-300 p-3 rounded font-mono mb-2 h-10 flex items-center">{idx >= 0 ? steps[idx].m : '點擊開始...'}</div>
                    <div className="text-xs font-bold text-slate-500">結果：<span className="text-blue-600">[{res.join(', ')}]</span></div>
                </div>
            );
        };
...
// 使用 Blob 技術將 HTML 代碼轉換為可以在 iframe 運行的 URL
