...
<body>
    ...
    <script type="text/babel">
        ...
        const App = () => {
            ...
            const [logs, setLogs] = useState(["準備就緒，點擊下一步開始模擬..."]);

            const steps = useMemo(() => {
                const s = []; const curR = []; let curT = [];
                const findNode = (r, id) => {
                    if (!r) return null; if (r.id === id) return r;
                    return findNode(r.left, id) || findNode(r.right, id);
                };
                const getR = (n, ts) => {
                    const t = ts.find(i => i.from === n.id);
                    return t ? findNode(INITIAL_TREE, t.to) : n.right;
                };

                let curr = INITIAL_TREE;
                while (curr) {
                    s.push({ c: curr.id, p: null, ts: [...curT], r: [...curR], m: "檢查節點 " + curr.val });
                    if (!curr.left) {
                        curR.push(curr.val);
                        s.push({ c: curr.id, p: null, ts: [...curT], r: [...curR], m: curr.val + " 無左子樹，加入結果並移至右側" });
                        curr = getR(curr, curT);
                    } else {
                        let p = curr.left;
                        while (getR(p, curT) && getR(p, curT).id !== curr.id) p = getR(p, curT);
                        if (!getR(p, curT)) {
                            curT.push({ from: p.id, to: curr.id });
                            s.push({ c: curr.id, p: p.id, ts: [...curT], r: [...curR], m: "建立線索：" + p.val + " -> " + curr.val });
                            curr = curr.left;
                        } else {
                            curT = curT.filter(i => !(i.from === p.id && i.to === curr.id));
                            curR.push(curr.val);
                            s.push({ c: curr.id, p: p.id, ts: [...curT], r: [...curR], m: "斷開線索，訪問 " + curr.val + " 並移至右側" });
                            curr = getR(curr, curT);
                        }
                    }
                }
                s.push({ c: null, p: null, ts: [], r: [...curR], m: "遍歷完成！" });
                return s;
            }, []);
            ...
            const handleNext = () => {
                if (stepIdx < steps.length - 1) {
                    const n = stepIdx + 1;
                    setStepIdx(n);
                    setCurrentId(steps[n].c);
                    setPreId(steps[n].p);
                    setThreads(steps[n].ts);
                    setRes(steps[n].r);
                    setLogs(prev => [steps[n].m, ...prev].slice(0, 3));
            ...
            const findPos = (n, id, x, y, dx) => {
                if (!n) return null; if (n.id === id) return { x, y };
                return findPos(n.left, id, x-dx, y+60, dx/1.7) || findPos(n.right, id, x+dx, y+60, dx/1.7);
            };

            const renderNode = (n, x, y, dx) => {
                if (!n) return null;
                const isC = currentId === n.id;
                const isP = preId === n.id;
                return (
                    <g key={n.id}>
                        {n.left && <line x1={x} y1={y} x2={x-dx} y2={y+60} stroke="#e2e8f0" strokeWidth="2" />}
                        {n.right && <line x1={x} y1={y} x2={x+dx} y2={y+60} stroke="#e2e8f0" strokeWidth="2" />}
                        <circle cx={x} cy={y} r="18" fill={isC ? '#3b82f6' : (isP ? '#f59e0b' : '#fff')} stroke="#94a3b8" strokeWidth="2" className="node-transition" />
                        <text x={x} y={y} dy="5" textAnchor="middle" fill={isC || isP ? "white" : "#475569"} style={{fontSize: '12px', fontWeight: 'bold'}}>{n.val}</text>
                        {renderNode(n.left, x-dx, y+60, dx/1.7)}
                        {renderNode(n.right, x+dx, y+60, dx/1.7)}
                    </g>
                );
            };

            return (
                <div className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-screen">
                    <div className="flex justify-between items-center mb-4">
                        <div className="text-sm font-bold text-slate-700">Morris Traversal 視覺化</div>
                        <div className="flex gap-2">
                            <button onClick={() => setIsPlaying(!isPlaying)} className="px-3 py-1 bg-blue-600 text-white text-xs rounded">{isPlaying ? '暫停' : '播放'}</button>
                            <button onClick={handleNext} className="px-3 py-1 bg-slate-100 text-xs rounded">下一步</button>
                            <button onClick={() => window.location.reload()} className="px-3 py-1 border text-xs rounded">重置</button>
                        </div>
                    </div>
                    <div className="flex-1 bg-slate-50 rounded-lg border border-slate-100 flex justify-center items-center overflow-hidden mb-4">
                        <svg width="100%" height="240" viewBox="0 0 400 300">
                            {renderNode(INITIAL_TREE, 200, 40, 100)}
                            {threads.map((t, i) => {
                                const f = findPos(INITIAL_TREE, t.from, 200, 40, 100);
                                const to = findPos(INITIAL_TREE, t.to, 200, 40, 100);
                                if (!f || !to) return null;
                                return <path key={i} d={"M"+f.x+" "+f.y+" Q"+(Math.max(f.x, to.x)+40)+" "+((f.y+to.y)/2)+" "+to.x+" "+to.y} fill="none" stroke="#10b981" strokeWidth="2" strokeDasharray="4" className="opacity-60" />;
                            })}
                        </svg>
                    </div>
                    <div className="bg-slate-900 text-blue-300 p-3 rounded font-mono text-xs mb-2 h-16 flex flex-col justify-center">
                        {logs.map((log, i) => <div key={i} className={i === 0 ? "text-green-400" : "opacity-50"}>{i === 0 ? "> " : "  "}{log}</div>)}
                    </div>
                    <div className="text-xs font-bold text-slate-500">遍歷結果: <span className="text-blue-600">[{res.join(', ')}]</span></div>
                </div>
            );
        };
